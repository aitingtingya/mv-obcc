...
- direct drops from the vault or local filesystem: images become attachments, regular files become live `@file` references, and folders become native DSH directory references;
...
- mv-agent environment installation accesses the network only after the user clicks an action. Vault-installed Node.js, DSH, and pnpm runtimes live under `<vault>/mv-aide/dsh/`; the settings can independently pin `<vault>/mv-aide/dsh/home` as the DSH data directory, which does not change where DSH itself is installed, and turning it off keeps the official `$DSH_HOME` / `~/.dsh` behavior. The three independently managed DSH plugins and their internal compatibility library live in the selected DSH web profile; processes started by mv-AIDE also write secret-free PID/port/identity fingerprints under the same `DSH_HOME/.mv-aide/runtime-owners/`, used only to avoid adopting or stopping another DSH. The compatibility library has no Cordis entry or standalone patch row and is not loaded as a fourth plugin. Downloads, npm caches, and installer scripts use an operation-scoped temporary workspace that is removed when the action ends. Global installation or in-place upgrade uses a native administrator prompt and never silently falls back to the vault.
