...
## 规范五：功能不得由文件保存 / 自动保存事件驱动 (No Save-Event-Driven Features)

1. **设计初衷**：
   Obsidian 对编辑内容是**即时自动保存**的，"用户手动保存"这一事件概念在 API 层并不存在（`editor:save-file` 等不适用）。任何以"保存后触发"为语义的功能设计都是错误的：它要么永不触发，要么在用户无感知地连续自动保存时被反复意外触发。
2. **规范要求**：
   - 严禁通过监听 `vault.on("modify")` / `editor:save-file` / Ctrl+S 挂钩来实现"保存即执行 X"。
   - 功能触发一律来自**显式用户动作**（命令面板 / 菜单 / 快捷键 / 编辑器事件如选区变化、打开文件）或插件自身生命周期事件。
   - `vault.on("modify")` 仅允许用于**幂等的后台维护**（如缓存、索引），且不得触发任何面向用户的副作用命令。

---

## 规范六：新功能必须模块化 (Modular New Features)

1. **设计初衷**：
   既有功能（source-assist、终端、行内补全、LLM 桥、diff 等）是长期演进、受既有测试钉死的稳定模块。新功能若嵌入或改写既有功能内部，会破坏其稳定边界、放大回归面，并使功能之间互相竞争。
2. **规范要求**：
   - 任何新功能在自己所在的**层级**必须是模块化的：功能逻辑集中在独立目录/文件中，不得散布或嵌入既有功能内部。
   - 严禁改写 **UI（设置面板 / 视图渲染）之外**的既有功能代码——包括但不限于 `src/source-assist/*`、`src/terminal/*`、`src/inline-completion/*`、`src/llm-*.ts` 等的内部逻辑。
   - 接入既有系统只允许**纯新增挂点**：`BridgeSettings` / `constants` 新增字段与默认值、设置面板新增渲染挂载、视图类新增公开方法、`main.ts` 新增命令/扩展注册；不得重构或改写既有逻辑。
   - 新功能不得与既有功能**竞争**：不拦截既有功能处理的事件（除非由显式用户动作触发），不使用或覆写既有功能的内部状态。
