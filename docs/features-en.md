...
- **Equal to the current mv-AIDE version**: fully checks the marker, file set, fingerprint, patch registration, and actual DSH load. Missing or damaged state shows Repair. A different fingerprint at the same version is reported as a build conflict and can be replaced only by an explicit Update, or automatically in the background while plugin auto-update is enabled (the never-downgrade rule for higher versions is unaffected).
...
- A recall round terminates irreversibly on the first caret or editing gesture: pressing `←`/`→`/`Home`/`End` inside the composer (with any modifier combination), clicking the composer, pasting, cutting, starting an IME composition, or an actual selection that disagrees with the round's expected end-of-text caret immediately exits recall, leaving the content and caret exactly as the user's action left them without interrupting the action itself. Afterwards `↑`/`↓` fall back to DSH's native caret movement, and only emptying the draft re-arms recall. The plugin's own caret placement distinguishes itself from user moves through a round-bound expected-selection comparison, never a time-based suppression.
...
Completion candidates (names, `@group`, and `@group -p`, with member commands in the detail line) and the execution preview share the suggestion list: after the “▶” run row, each step lists its name, 🔒 (protected), `[-n]` (new terminal), and command text (collapsed past 8 steps), with filtered members marked “Skipped by protection rules”. **Enter always executes** (empty input = default order, typed input = the expression, regardless of the highlighted row; Enter during IME composition is ignored), **Tab accepts the highlighted completion** (the palette reopens with the text and cursor preserved), Esc cancels; invalid expressions show an error row instead of a run row, so nothing partially executes. Nothing is saved or executed before confirmation. If task definitions change before specified execution, a notice appears and the palette reopens against the refreshed definitions for a second confirmation.
...
### Recall stops walking back mid-way

If any caret or editing gesture happened during recall (moving the caret with `←`/`→`, clicking the composer, pasting, cutting, an IME composition, and so on), the recall round has already terminated. A later `↑` then acts as the native caret movement — this is expected, not a defect. Emptying the draft re-arms `↑` to start again from the newest message.
