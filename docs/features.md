...
补全候选（名称、`@分组` 与 `@分组 -p`，附成员命令小字）与执行预览同列在建议区：「▶」执行行之后逐步列出名称、🔒（受保护）、`[-n]`（新建终端）与命令文本（超过 8 步折叠计数），被过滤成员标注「因保护规则跳过」。Enter 选中补全即插入（选择器重开后保留文本与光标继续输入）、选中执行行即执行，Esc 取消；表达式有误时建议区直接给出错误说明，且不存在可执行行，不会部分执行。确认前不保存文件、不运行命令。指定执行时若文件指令发生变化，会以 Notice 提示、按最新指令重开选择器并要求再次确认。
