...
| 9 | Git | 开；编辑标记与行归属默认关 | 原生 Git 工作区、命令面板动作、diff、历史、冲突解决 |
...
- **视图**：命令面板提供「打开 mv-agent」「停止 mv-agent」「重启 mv-agent」，快捷键可在 Obsidian 快捷键设置中绑定。「停止 mv-agent」会关闭所有已打开的 mv-agent 界面并停止对应的 DSH 后台：停止结果如实通知，个别后台进程未能终止时提示会列出 PID 并说明端口仍被占用，不伪报成功。除此之外，DSH 后台不随插件或 Obsidian 重载退出；Obsidian 重开后会自动接回本 Vault 的原实例（同端口），只有「停止 mv-agent」才真正终止它。视图是一个自定义 Obsidian 视图：上方 iframe 直接嵌入 DSH Web 界面（无浏览器工具栏），底部是 Obsidian 侧状态栏。状态栏显示连接球、当前页面或文件、选区、端口和展开入口；展开后可查看 DSH 地址、桥接状态、选区范围与正文，并进入插件、技能、子 Agent 管理或外部浏览器。位置与选区勾选框分别控制下次消息是否携带对应快照。连接球直接根据当前视图 DSH 端点到本 Vault bridge 的真实 TCP 连接显示：检查中为灰色，已连接为绿色，确认断开为红色；每个视图独立探测，不依赖本 Vault 是否启动了共享 DSH，也不依赖环境检测缓存。连接状态最多每 15 秒探测一次（安装、升级或重启期间暂停），切换页面或重新打开视图时立即探测。
...
- **mv-agent 弹出窗口**：弹出（popout）窗口的位置与尺寸按 Vault 记忆，重载或重启后恢复到原几何（修复 Windows 上重载后总是居中的问题）；记录已移出屏幕的几何会被放弃并回退系统默认。启动时按「Vault + 会话」标记回收本 Vault 旧会话遗留的僵尸弹窗，不触碰其它 Vault 或应用的窗口，回收数量只写控制台。
- **视图自愈与加载失败**：端点失联时自动修复：实例还活着就重挂代理修复只读页面，实例没了就接回或拉起原实例并重连全部视图，重试按 5 秒起步、封顶 30 秒退避；「停止 mv-agent」之后不再介入。单视图地址解析失败会按 0.5 秒间隔自动重试约 10 秒，仍失败则停在该视图的失败态并显示原因，右键标签「刷新界面」可重新解析；任何路径都不会让 iframe 直连带 token 的原始地址。
- **授权模式与回环代理**：当 DSH 端点带 launch token 授权（Alpha）时，mv-agent 的 iframe 实际加载的是 mv-AIDE 自有的回环反向代理地址（端口由系统分配，仍只绑定 `127.0.0.1`）：启动令牌只在当前进程内完成一次交换以换取会话 Cookie，从不落盘或记入日志；Cookie 不出现在页面。为支持重连，插件会把 Cookie 经系统安全存储（Electron safeStorage）加密后，按精确 loopback origin（端点）隔离保存到 `~/.mv-aide/dsh/web-auth/`（原子写入，目录 `0700`、文件 `0600`）；不再按运行身份区分，切换仓库版／全局版或升级 DSH 后登录态仍然有效，旧格式记录自动迁移。系统安全存储不可用（含 Linux `basic_text`）时不做明文降级，登录态仅在本次 Obsidian 运行期间有效并会明确提示。`/api/` 下的 WebSocket 全部经代理隧道；上游重启导致 Cookie 失效时会自动重新兑换。需要鉴权的实例重连时，先按保存的 endpoint 与运行身份恢复 Cookie 并完成真实验证；无凭据、凭据过期或验证失败时停在原实例并报告「DSH 要求重新授权」，不会另起第二实例争抢会话——此时使用命令面板「使用 DSH 授权 URL 重新连接」，粘贴原 DSH 进程启动时输出的完整授权 URL 即可重连原实例。无鉴权端点（预览版）不建代理也不读写鉴权缓存，保持直连。「用浏览器打开」与「复制 DSH 地址」给出的是原始启动地址（顶层导航可正常落 Cookie），永远不会是代理地址。
...
- **等于当前 mv-AIDE**：完整核对 marker、文件集合、指纹、patch 和实际加载；缺失或损坏时显示“修复”。同版本但指纹不同显示构建冲突，只能由显式“更新”替换；开启“插件自动更新”时会在后台自动替换（更高版本永不降级的规则不受影响）。
...
- 召回轮次一经任何光标或编辑操作即不可逆地终止：在输入框内按 `←`/`→`/`Home`/`End`（含任意修饰键组合）、点击输入框、粘贴、剪切、开始 IME 组合，或实际选区偏离召回轮次预期的文末光标时，立即退出召回，内容与光标原样保留在用户操作后的状态，不打断用户操作本身；此后 `↑`/`↓` 恢复 DSH 原生光标移动，只有清空草稿才能重新进入召回。插件自身放置光标使用与轮次绑定的预期选区对比来区分自身操作与用户操作，不依赖任何基于时间的抑制。
...
- 按键直通开启时，终端聚焦后的 Ctrl、Alt、功能键和方向键组合优先发给终端程序。命令面板、设置页或确认框等弹窗打开期间按键让行给弹窗：面板里的 Enter 不会泄入终端 shell，关闭弹窗的那次按键同样不会落到终端。
...
执行命令面板的「运行 mv-run 指令」对当前 Markdown 视图打开命令面板式选择器（打开即聚焦，全程键盘操作）：命令在终端、PDF、网页等任何界面聚焦时都能在命令面板找到，当前没有打开的 Markdown 视图时会以 Notice 提示。快捷键可在 Obsidian 快捷键设置中绑定；命令 ID 保持 `run-file-bottom-command`，既有绑定不受影响。前缀在当前 profile 的「指令注释前缀」设置中配置：分号分隔多个前缀，`<!--` 与 `/*` 前缀会把行尾的 `-->` / `*/` 一并剥离，留空则禁用该类型；该设置行右侧的文档按钮按界面语言打开本手册 GitHub 版的 mv-run 章节（中文界面打开 `docs/features.md#mv-run`，英文界面打开 `docs/features-en.md#mv-run`）。入口校验在打开选择器前完成：该类型未配置前缀、文件中没有指令、或任一指令行存在语法错误时选择器不会打开，Notice 分别提示未配置前缀、未找到指令，或以 `行号:列号` 指出语法错误位置。

选择器输入为空时第 1 行即「默认执行」（高亮选中）：直接回车按文件顺序执行所有没有 `--protect` 的命令；第 2 行起列出全部可用的名称与 `@分组` 补全，末尾附一条示例提示行。一旦输入名称与分组列表，第 1 行切换为「▶」加已输入序列的执行行，其后为补全候选与实际执行顺序预览（含被过滤的成员）；预览、提示与错误行不可执行，选中后原样保留输入重新打开选择器。
...
补全候选（名称、`@分组` 与 `@分组 -p`，附成员命令小字）与执行预览同列在建议区：「▶」执行行之后逐步列出名称、🔒（受保护）、`[-n]`（新建终端）与命令文本（超过 8 步折叠计数），被过滤成员标注「因保护规则跳过」。**Enter 永远执行**（空输入=默认执行，有输入=按表达式执行，与高亮行无关；输入法组词中的 Enter 不触发），**Tab 接受高亮补全**（插入后选择器重开并保留文本与光标），Esc 取消；表达式有误时建议区直接给出错误说明，且不存在可执行行，不会部分执行。确认前不保存文件、不运行命令。指定执行时若文件指令发生变化，会以 Notice 提示、按最新指令重开选择器并要求再次确认。
...
默认开启。调用本机安装的 Git 管理**当前 Vault 所属仓库**；不内置 Git 引擎、不关联 DSH、不自动提交／同步／联网、不修改全局 Git 配置。

### 设置项

| 设置 | 默认 | 说明 |
| --- | --- | --- |
| 启用 Git 集成 | 开 | 关闭后不注册视图入口的主动行为，命令执行前提示已关闭 |
| Git 可执行文件路径 | 空 | 留空使用系统 PATH；自定义路径下次调用生效 |
| 仓库检测与诊断 | — | 显示版本、真实仓库根目录、分支与进行中的操作；未初始化时提供「初始化当前 Vault」 |
| 工作区位置 | 左侧面板 | 可选右侧、独立完整标签页、分栏 |
| diff 打开位置 | 中间主区 | diff 与冲突编辑器的落点：主区新标签页／沿 Git 工作区分屏／右侧栏／左侧栏 |
| diff 布局 | 自动 | 并排／行内，或按面板宽度自动 |
| 文件树视图 | 开 | 按目录折叠分组；关闭为平铺列表 |
| 自动刷新仓库状态 | 开 | 文件事件与定时刷新只使缓存失效；无人查看时停止周期扫描 |
| 状态栏显示 Git 状态 | 开 | 分支、领先／落后、进行中操作；点击打开工作区 |
| 操作结果通知 | 开 | 写操作成功后右上角气泡提示；失败通知始终弹出，与工作区是否打开无关 |
| 编辑器修改标记 | 关 | 行号槽标注相对 HEAD 的添加／修改／删除 |
| 当前行归属提示 | 关 | 行尾显示该行最近提交摘要 |
| 拉取策略 | 遵循仓库配置 | 仓库未配置时仅快进；可显式 merge／rebase |
| 提交消息模板 | 空 | 纯文本初始内容，不执行脚本 |

### 能力表（工作区与命令面板双端同构）

所有功能封装在 `src/git/`，经统一动作注册表暴露：稳定动作 ID + 目标类型 + 参数收集 + 可执行条件 + 确认流程 + 执行器。GUI 右键携带已选目标；命令面板使用同一动作，无目标时打开可搜索选择器。`scripts/check-git-coverage.mjs` 静态校验双端可达性（已挂入 `npm run verify`）。

| 对象 | 操作 |
| --- | --- |
| 仓库 | 初始化、刷新、查看状态／输出、取消进行中的命令、编辑 `.gitignore`、设置本仓库作者信息 |
| 工作区文件／目录 | 打开、diff、暂存、丢弃（确认）、忽略、文件历史、复制路径、系统定位；多选与目录操作 |
| 暂存文件／目录 | 暂存 diff（HEAD→索引）、取消暂存（含反向预览）；单项／多选／全部 |
| 差异块／选中行 | 暂存、取消暂存；基于 Git 原生 patch 校验，索引变化即拒绝 |
| 提交 | 提交暂存、暂存全部并提交、提交并推送、暂存全部并提交推送、提交并同步、修改最新提交、撤销最新提交保留改动；提交主按钮记住最近一次下拉动作 |
| 历史提交节点 | 详情／变更文件、与父提交／HEAD／上游／共同祖先／任意版本比较、复制 SHA／消息、检出（明示 detached HEAD）、创建分支／标签、cherry-pick、revert、reset（soft/mixed/hard 附影响说明）、重置当前分支到此处并重载 Obsidian、合并、变基到此处 |
| 批量节点 | 「节点批量操作」按钮开启后显示提交复选框（默认隐藏）；多选 cherry-pick／revert：按拓扑旧→新排序、预览顺序后执行、冲突即停并报告已完成／剩余 |
| 本地分支 | 创建、切换、重命名、删除（确认）、设置上游、发布、比较、合并、变基、查看该分支提交历史 |
| 远程分支 | 抓取、检出跟踪分支、设为上游、比较、合并／变基、确认后删除远程分支 |
| 标签 | 轻量／附注创建、查看、比较、检出、从标签建分支、推送、删除本地／指定远程标签 |
| stash | 创建（可选含未跟踪或仅暂存内容）、查看、应用、弹出、删除、从 stash 建分支 |
| 远程与同步 | 添加／修改／重命名／删除远程；抓取指定远程、获取（`git fetch --all`，下载远端全部分支且不触碰工作区，等同 SourceTree 的「获取」）、清理失效引用、拉取、推送、首次设上游、同步、带 lease 强制推送 |
| 进行中操作 | merge／rebase／cherry-pick／revert 的继续与中止；rebase／cherry-pick／revert 另支持跳过（Git 本身无 merge --skip）；冲突三方编辑与标记已解决 |
| 历史恢复 | reflog 查看、从记录创建恢复分支（复用统一确认流程） |
| 文件历史与归属 | 跟随重命名的历史、历史版本 diff、恢复指定版本到工作区（确认）、当前行归属查询 |

### 语义约定

- 有暂存内容只提交暂存；无暂存时列出范围询问是否暂存全部。
- 「提交并推送」= 提交→推送；「同步」= 拉取→推送；「提交并同步」= 提交→拉取→推送。失败即停止后续步骤并报告部分成功。
- merge 提交的 cherry-pick／revert 必须显式选择主线父提交。
- 检出提交始终进入 detached HEAD，并在动作名与确认中明示。
- revert 创建反向提交，不伪装成 reset；reset 明确区分 soft/mixed/hard。
- 破坏性动作（丢弃、删除、reset、远程删除、强制推送、覆盖恢复）均有目标明确的确认，无默认强制行为。

### 可靠性

- 参数数组调用、NUL 分隔机器输出、`--` 与 `--literal-pathspecs` 保护特殊文件名（stash 子命令按上游缺陷豁免，否则 `--include-untracked` 清理失败）。
- 写操作串行进入队列；复合动作整体入队；取消只终止本模块进程；外部 `index.lock` 不擅删，报错后重读真实状态。
- 沿用用户 credential helper、SSH agent、签名与 hooks；认证／hooks 失败原样报错，不伪报成功。
- 行／块暂存以 `git diff` 与 `git ls-files` 指纹为依据；文件内容写回绑定身份与字节修订，外部改动时保留草稿并提示。
- 冲突编辑器显示基线、双方真实来源（merge／cherry-pick／revert／rebase 分别标注，rebase 语义正确互换）与可编辑结果；保存与标记已解决分离。
- 历史行显示哈希、标题、分支／标签徽章与绝对时间；写操作成功／失败均弹出全局气泡（成功气泡可在设置中关闭）。
- 工作区分区顺序：文件分组 → 分支 → 提交历史 → 标签 → 工作进度 → 远程；历史区经「选择分支」菜单按当前分支／全部分支／任意分支过滤，分支右键可直接跳转到该分支历史。
- 多窗口：对话框、diff、冲突视图绑定发起窗口；各窗口选中集、输入草稿、滚动位置互不覆盖；共享同一仓库快照与写队列。

### 边界

- 只管理当前 Vault 所属仓库；属于上层仓库时展示真实根目录并需确认，不静默创建嵌套仓库。
- 当前仓库是 worktree 时正常使用真实 git-dir；子模块正确显示但不自动递归提交。
- 不含定时自动提交／同步、GitHub PR、云端建仓、其它 worktree 管理、交互式变基编排。
- 二进制／特殊条目支持整文件操作，不提供精细文本操作。
...
- 每个 Git 动作一条 `Git: …` 命令（稳定 ID `git-<action>`），与工作区右键菜单共用同一动作注册表；从命令面板发起时**一切后续输入都在面板内完成**：选项走模糊选择框、单行文本以已输入内容为首条建议（Enter 即提交）、确认走「确认／取消」两项选择（确认项附明细）、提交消息等多行参数在面板路径下为单行输入，打开即聚焦、全程纯键盘、Esc 取消。当前文件命令自动以活动文件为目标。
...
| Git | 是 | 是 | 是 | 需本机安装 Git；macOS 已实测，Windows/Linux 需对应平台验收 |
...
| `~/.mv-aide/dsh/web-auth/` | 经系统安全存储加密的 DSH Web 登录 Cookie，按精确 loopback origin（端点）隔离，旧版按运行身份区分的记录自动迁移；不含启动令牌、明文 Cookie 或聊天内容；安全存储不可用时不明文落盘 |
...
6. 报「当前 DSH 版本不支持 --no-open（该选项自 0.1.0-rc.8 起提供）」时，说明当前 DSH 早于 0.1.0-rc.8，请在 mv-agent 设置的 DSH 层升级。插件会按二进制探测并缓存该能力：支持时传 `--no-open` 避免启动弹浏览器，旧版本自动省略（旧版本本就不会弹浏览器）。
...
召回过程中只要发生过一次光标或编辑操作（方向键左右移动光标、点击输入框、粘贴、剪切、IME 组合等），召回轮次就已终止；此时再按 `↑` 表现为原生光标移动属于预期行为，不是缺陷。清空草稿后 `↑` 会重新从最新一条开始召回。
