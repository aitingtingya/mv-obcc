...
function writeJson(filePath: string, value: ClaudeSettingsDocument): void {
  ...
  const temporary = `${filePath}.mv-aide-hook.tmp`;
...
export function managedTerminalMarkerCommand(platform: NodeJS.Platform): string {
  if (platform === "win32") {
    return [
      ...
      "Write-Output ('{\"suppressOutput\":true,\"terminalSequence\":\"\\u001b]0;mv-aide:'+$s+'\\u0007\"}');",
      `# ${MANAGED_HOOK_MARKER}`,
    ].join(" ");
  }
  return [
    `printf '{"suppressOutput":true,"terminalSequence":"\\\\u001b]0;mv-aide:%s\\\\u0007"}\\n' "$CLAUDE_CODE_SESSION_ID";`,
    `: ${MANAGED_HOOK_MARKER}`,
  ].join(" ");
}
